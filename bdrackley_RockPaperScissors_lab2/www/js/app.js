...
lab2.controller('player1', function(processMove, $rootScope, $scope, $ionicModal, $ionicLoading, $ionicViewSwitcher, $state ,$stateParams, navigation) {
  ...
  $scope.onNext = function (choice) {
    ...
    $scope.player1_choice = choice;
    //console.log("Player1 choice is: " + $scope.player1_choice);

    if ($rootScope.wasInStartState != true && $scope.player1_wins >= 2) // Check if previous state was start state using variable on $rootScope
      // Additionally check if player 1 has best 2/3
    {
      if(window.plugins && window.plugins.nativepagetransitions) {
        ...
        $stateParams.winner = 1;
        //console.log("Winner: " + $stateParams.winner);
        $state.go("results", {
          'winner': $stateParams.winner
        });
      }
    }
    else if ($rootScope.wasInStartState != true && $scope.player2_wins >= 2) // Check if previous state was start state using variable on $rootScope
    // Additionally check if player 2 has best 2/3
    {
      ...
      if(window.plugins && window.plugins.nativepagetransitions) {
        ...
        $state.go("player2", {
          'player1_choice' : $scope.player1_choice,
...
lab2.controller('player2', function(processMove, $rootScope, $scope, $ionicModal, $ionicLoading, $ionicViewSwitcher, $state, $stateParams, navigation) {
  ...
  $scope.onNext = function (choice) {
    ...
    $stateParams.player2_choice = choice;
    //console.log("Player2 choice is " + $stateParams.player2_choice);
    //console.log("Player1 choice is " + $stateParams.player1_choice);

    /* TODO: Use processMove factory instance to check if player1 has a win */
    $scope.win = $scope.processMove.checkWins($stateParams.player1_choice, $stateParams.player2_choice);
    ...
    if ($scope.player1_wins >= 2) // Check if player1 has best 2/3
    {
      if(window.plugins && window.plugins.nativepagetransitions) {
        // Ignore this block for now; this is for native transitions (More on Wednesday 2/22/17) //
      }
    ...
    else if ($scope.player2_wins >= 2) // Check if player2 has best 2/3
    {
      if(window.plugins && window.plugins.nativepagetransitions) {
        // Ignore this block for now; this is for native transitions (More on Wednesday 2/22/17) //
      }
